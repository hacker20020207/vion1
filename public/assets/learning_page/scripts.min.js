...
(function ($) {
  ...
  $('body').on('click', '.tab-item', function () {
    ...
    if (!$this.hasClass('active')) {
      ...
      var watermark = $(this).attr('data-video-watermark');
      //
      ...
      if (type !== 'assignment') {
        addContentLoading();
        handleContent(id, type, description, watermark);
      }
    }
  });
  ...
  function handleContent(itemId, itemType, description=null, watermark=null) {
    var data = {
      type: itemType,
      id: itemId
    };
    $.post('/course/learning/itemInfo', data, function (result) {
      if (itemType === 'session') {
        ...
        handleFileHtml(result.file, description, watermark); // if file is downloadable
  ...
  function handleFileHtml(file, description='', watermark='') {
    if (file.online_viewer && file.online_viewer !== '0' || file.downloadable && file.downloadable !== '0') {
      if (file.online_viewer && file.online_viewer !== '0') {
        var html = "<div class=\"d-flex flex-column position-relative p-10 h-100\" id=\"learning_course_vedio\">";
        html += "<div class=\"video-watermark-overlay\" id=\"vedio_watermark_overlay\">"+watermark+"</div>\n <iframe src=\"/ViewerJS/index.html#".concat(file.file_path, "\" class=\"file-online-viewer rounded-sm ").concat(file.downloadable && file.downloadable !== '0' ? 'has-download-card' : '', "\" frameborder=\"0\" allowfullscreen></iframe>");
        ...
        if (file.is_video) {
          var $html = "<div class=\"js-video-and-download d-flex flex-column p-10 h-100\" id=\"learning_course_vedio\">";
  ...
  document.addEventListener('fullscreenchange', () => {
      var vedio_watermark_overlay = document.getElementById('vedio_watermark_overlay')
      console.log([learning_course_vedio, learning_course_vedio.querySelector('video')])
      document.fullscreenElement.appendChild(vedio_watermark_overlay); // joylashtirib qo'yiladi
  })
